...
GET /v2/library/nginx/manifests/latest
  ...
  └─ ③ どちらも無い
        → 順序フォールバック: docker.io → ghcr.io → quay.io
           ...
           応答しない上流は待ち時間（既定 5 秒）で打ち切って次へ進む（REQ-0057 / 0059）
...
| `ci.yml` | `cargo fmt` / `cargo clippy` / `cargo test`、`Cargo.toml` が宣言する最小 Rust 版で組めるかの確認、amd64・arm64 双方のビルド（REQ-0050 / 0053） |
