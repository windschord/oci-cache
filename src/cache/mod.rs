...
//! （マニフェストのキャッシュと再検証）。取得元の決定（`crate::routing`）
//! が終わった後、この段階で実際に blob・マニフェストを取得・保存する。
//!
//! 認証情報を用いた問い合わせ（REQ-0016）と、非公開イメージを保存しない
//! 判定（REQ-0013 / REQ-0019）、OCI Image Layout による永続化
//! （REQ-0020 〜 REQ-0022）は、次のフェーズ（README の実装順序 3.）で扱う。
